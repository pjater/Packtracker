...
function renderScanResultsModal() {
  ...
  title.textContent = translate("scanResults", "Scan Results");

  const subtitle = document.createElement("div");
  subtitle.className = "modal-subtitle";
  subtitle.textContent = scanSession.failedToOpen
    ? (scanSession.errorMessage || translate("scanCouldNotOpenFolder", "Could not open the selected folder."))
    : `${scanSession.folderPath} - ${scanSession.rows.length} ${scanSession.scanConfig.fileKindLabel} ${translate("scanFoundSuffix", "found")}`;
  ...
  closeButton.setAttribute("aria-label", translate("closeScanResults", "Close scan results"));
  ...
  betaTag.textContent = translate("experimental", "Experimental");
  const betaText = document.createElement("span");
  betaText.textContent = translate("scanExperimentalBody", "Scan results may be incomplete or inaccurate. It is safer to add items manually or double-check everything before adding it to your profile.");
  ...
  SCAN_PROVIDER_PREFERENCES.forEach((entry) => {
    ...
    button.addEventListener("click", () => {
      ...
      renderScanResultsModal();
      if (typeof namespace.showToast === "function") {
        namespace.showToast(`${translate("scanProviderPreferenceSet", "Scan provider preference set to")} ${entry.label}.`, "success");
      }
    });
  ...
  addAllButton.textContent = scanSession.addingAll ? translate("adding", "Adding...") : translate("addAllFound", "Add All Found");
...
function renderScanRow(row) {
  ...
  if (row.status === "added") {
    ...
    searchButton.textContent = row.searching ? translate("searching", "Searching...") : translate("searchAction", "Search");
...
async function addInlineResult(rowId) {
  ...
  if (typeof resolveAndAdd !== "function") {
    row.status = "not-found";
    row.statusText = translate("addFlowNotReady", "Add flow is not ready yet. Please try again.");
    row.searchResult = null;
    scanSession.activeInlineRowId = null;
    renderScanResultsModal();
    if (typeof namespace.showToast === "function") {
      namespace.showToast(translate("addFlowNotReady", "Add flow is not ready yet. Please try again."), "danger");
    }
    return;
  }
...
function renderInlineSearchResult(row) {
  ...
  title.textContent = `${row.searchResult.project.title || row.searchResult.project.name} ${translate("by", "by")} ${row.searchResult.project.author || translate("unknownAuthor", "Unknown author")}`;

  const meta = document.createElement("div");
  meta.className = "scan-inline-meta";
  meta.textContent = `${resolveSearchSourceLabel(row.searchResult.project.source)} - ${row.searchResult.version.version_number || translate("unknown", "Unknown")} - ${formatLoaders(row.searchResult.version.loaders)} - ${formatMcVersion(row.searchResult.version.game_versions)}`;

  const description = document.createElement("div");
  description.className = "scan-inline-description";
  description.textContent = row.searchResult.project.description || translate("noDescriptionProvided", "No description provided.");
  ...
  addButton.textContent = translate("addToProfile", "+ Add to profile");
  ...
  dismissButton.textContent = translate("notThisOne", "Not this one");

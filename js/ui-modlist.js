(function attachModListModule() {
  ...
  function shouldShowButton(key) {
    return typeof isButtonVisible !== "function" || isButtonVisible(key);
  }
...
function renderProfileView() {
  ...
  if (!isFavoritesProfileId(AppState.activeProfileId)) {
    ...
    shareButton.addEventListener("click", async () => {
      try {
        ...
        if (typeof namespace.showToast === "function") {
          namespace.showToast(translate("shareLinkCopied", "Share link copied!"), "success");
        }
      } catch (error) {
        if (typeof namespace.showToast === "function") {
          namespace.showToast(translate("shareLinkCopyFailed", "Could not copy share link"), "danger");
        }
      }
    });
    ...
    if (shouldShowButton("shareProfile")) {
      headerActions.appendChild(shareButton);
    }
    if (shouldShowButton("scanMinecraftFolder")) {
      headerActions.appendChild(scanButton);
    }
...
function renderTabContent(tab) {
  ...
  if (!isFavoritesView) {
    ...
    downloadButton.addEventListener("click", async () => {
      ...
      try {
        ...
        if (typeof namespace.showToast === "function") {
          namespace.showToast(error instanceof Error ? error.message : translate("zipDownloadFailed", "ZIP download failed"), "danger");
        }
      } finally {
    ...
    if (shouldShowButton("addManual")) {
      buttons.appendChild(manualButton);
    }
    if (shouldShowButton("updateContent")) {
      buttons.appendChild(updateButton);
    }
    if (shouldShowButton("downloadZip")) {
      buttons.appendChild(downloadButton);
    }
    buttons.appendChild(createViewToggle(tab));
    ...
    const clearItemsButton = createButton(translate("clearItems", "Clear items"), "btn-danger btn-small");
    clearItemsButton.replaceChildren(
      createIconLabelContent(
        "\uD83D\uDDD1",
        translate("clearItems", "Clear items"),
        "btn-icon-trash"
      )
    );
    clearItemsButton.addEventListener("click", () => {
      showClearItemsModal(profile.id, tab);
    });
    if (shouldShowButton("clearItems")) {
      buttons.appendChild(clearItemsButton);
    }
  ...
  if (isFavoritesView && shouldShowButton("downloadZip")) {
    buttons.append(downloadButton, createViewToggle(tab), editLayoutButton);
  } else if (isFavoritesView) {
    buttons.append(createViewToggle(tab), editLayoutButton);
  }
...
function renderModCard(mod, profileId, isEditing = false) {
  ...
  const includedMods = Array.isArray(mod?.includedMods) ? mod.includedMods : [];
  const isModpack = mod?.projectType === "modpack";
  ...
  author.textContent = `${translate("by", "by")} ${mod.author || translate("unknownAuthor", "Unknown author")}`;
  ...
  if (isModpack) {
    badges.appendChild(createBadge("badge-version", "Modpack"));
  }
  ...
  description.textContent = mod.description || translate("noDescriptionProvided", "No description provided.");

  if (isModpack && includedMods.length > 0) {
    titleStack.appendChild(createIncludedModsSection(includedMods));
  }
  ...
  meta.textContent = buildMetaLine([
    ...
    isModpack ? `${includedMods.length} mods inside` : "",
  ...
  actions.append(starButton, menuButton, deleteButton);
...
function renderPackCard(item, profileId, type, isEditing = false) {
  ...
  const author = document.createElement("div");
  author.className = "mod-author";
  author.textContent = `${translate("by", "by")} ${item.author || translate("unknownAuthor", "Unknown author")}`;
...
function checkItemCompatibility(item, profile) {
  ...
  const compatibleLoaders = resolveCompatibleProfileLoaders(profileLoader);
  const hasMatchingLoader = itemLoaders.length === 0
    || compatibleLoaders.some((loader) => itemLoaders.includes(loader));

  if (itemLoaders.length > 0 && !hasMatchingLoader) {
    warnings.push({
      level: "error",
      message: `This mod supports ${formatCompatibilityLoaderList(itemLoaders)} but your profile uses ${formatCompatibilityLoaderLabel(profileLoader)}.`,
    });
  }
...
/**
 * Resolves the loader labels that should count as compatible for one profile loader.
 *
 * Fabric and Quilt are treated as the same family because many projects expose
 * them interchangeably in upstream metadata.
 *
 * @param {string} profileLoader - Active profile loader.
 * @returns {Array<string>} Compatible loader labels.
 */
function resolveCompatibleProfileLoaders(profileLoader) {
  if (profileLoader === "fabric") {
    return ["fabric", "quilt"];
  }
  if (profileLoader === "quilt") {
    return ["quilt", "fabric"];
  }
  if (profileLoader === "forge") {
    return ["forge"];
  }
  if (profileLoader === "neoforge") {
    return ["neoforge"];
  }
...
/**
 * Formats one loader label for compatibility messages.
 *
 * @param {string} loader - Loader identifier.
 * @returns {string} Human-readable loader label.
 */
function formatCompatibilityLoaderLabel(loader) {
  if (loader === "neoforge") {
    return "NeoForge";
  }
  return loader ? capitalize(loader) : "Unknown loader";
}

/**
 * Formats a loader list for compatibility messages.
 *
 * @param {Array<string>} loaders - Supported loaders.
 * @returns {string} Human-readable loader summary.
 */
function formatCompatibilityLoaderList(loaders) {
  const uniqueLoaders = Array.from(new Set((Array.isArray(loaders) ? loaders : []).filter(Boolean)));
  return uniqueLoaders.map((loader) => formatCompatibilityLoaderLabel(loader)).join(", ");
}
...
function createChangeVersionButton(item, profileId, type) {
  const button = createButton(item.versionNumber || item.version
    ? translate("changeVersion", "Change version")
    : translate("pickVersion", "Pick version"));
  ...
  button.addEventListener("click", () => {
    void openVersionPickerModal(
      ...
      {
        profileId,
        projectType: type === "mod" ? (item?.projectType === "modpack" ? "modpack" : "mod") : type,
        projectName: item.name,
        mode: "update",
        confirmLabel: translate("updateToSelected", "Update to selected"),
      }
    );
  });
  return button;
...
async function openVersionPickerModal(projectId, currentVersionId, source, onConfirm, options = {}) {
  ...
  const versionFilters = {
    loader: (projectType === "mod" || projectType === "modpack") && profile.loader !== "vanilla" ? profile.loader : "",
  ...
  if (versions.error || !Array.isArray(versions) || versions.length === 0) {
    showTransientModal(
      translate("noVersionsAvailable", "No versions available"),
      translate("noInstallableVersionsBody", "{source} did not return any installable versions for this project.")
        .replace("{source}", resolveSourceLabel(source))
    );
    return;
  }
  ...
  const title = createModalTitle(
    options.mode === "update"
      ? translate("changeVersionForItem", "Change version for {item}")
        .replace("{item}", options.projectName || translate("thisItem", "this item"))
      : translate("addItemToProfile", "Add {item} to {profile}")
        .replace("{item}", options.projectName || translate("thisItem", "this item"))
        .replace("{profile}", profile.name)
  );
  const subtitle = createModalSubtitle(
    translate("chooseSourceVersionForProfile", "Choose a {source} version for {loader} {version}.")
      .replace("{source}", resolveSourceLabel(source))
      .replace("{loader}", profile.loader === "vanilla" ? translate("minecraft", "Minecraft") : capitalize(profile.loader))
      .replace("{version}", profile.mcVersion || translate("anyVersion", "any version"))
  );
  ...
  sortedVersions.forEach((version) => {
    ...
    name.textContent = version.version_number || translate("unknownVersion", "Unknown version");

    const badges = document.createElement("div");
    badges.className = "version-row-badges";
    badges.appendChild(createReleaseTypeBadge(version.release_type));
    if (recommendedVersion?.id === version.id) {
      badges.appendChild(createBadge("badge-source", translate("recommended", "Recommended")));
    }
    ...
    meta.textContent = buildMetaLine([
      ...
      `${translate("released", "released")} ${formatRelativeDate(new Date(version.date_published || 0).getTime())}`,
    ]);
  ...
  const confirmButton = createButton(
    options.confirmLabel || (options.mode === "update"
      ? translate("updateToSelected", "Update to selected")
      : translate("addSelectedVersion", "Add selected version")),
    "btn-accent"
  );
...
async function updateTrackedItemVersion(profileId, item, type, version) {
  ...
  if (type === "mod" && item?.projectType === "modpack") {
    patch.includedMods = await resolveModpackIncludedMods(item, version);
  }
  if (type === "mod") {
...
function showModNotesModal(profileId, modId) {
  ...
  const overlay = createModalOverlay();
  const modal = createModalCard();
  const title = createModalTitle(`${translate("notes", "Notes")} ${mod.name}`);
  const fieldGroup = document.createElement("div");
  fieldGroup.className = "form-group";
  const label = document.createElement("label");
  label.className = "form-label";
  label.textContent = translate("notes", "Notes");
  ...
  const actions = createActionRow();
  const cancelButton = createButton(translate("cancel", "Cancel"));
  const saveButton = createButton(translate("save", "Save"), "btn-primary");
  cancelButton.addEventListener("click", closeOverlays);
...
function showItemNotesModal(profileId, itemId, type) {
  ...
  const overlay = createModalOverlay();
  const modal = createModalCard();
  const title = createModalTitle(`${translate("notes", "Notes")} ${item.name}`);
  const fieldGroup = document.createElement("div");
  fieldGroup.className = "form-group";
...
function showAddManualModal(profileId, type) {
  ...
  const overlay = createModalOverlay();
  const modal = createModalCard();
  const title = createModalTitle(`${translate("add", "Add")} ${resolveTypeLabel(type)}`);
  ...
  const actions = createActionRow();
  const cancelButton = createButton(translate("cancel", "Cancel"));
  const addButton = createButton(translate("add", "Add"), "btn-primary");
  cancelButton.addEventListener("click", closeOverlays);
...
function createNoMatchesPanel() {
  ...
  panel.textContent = translate("noMatchingItems", "No matching items.");
  return panel;
}
...
function createFavoritesEmptyState(definition) {
  ...
  const text = document.createElement("div");
  text.className = "empty-state-text";
  text.textContent = translate("favoritesHelpText", "Use the star button on items in a normal profile to add them here.");
...
function showBulkUpdateModal(profileId, tab) {
  ...
  const modal = createModalCard();
  const title = createModalTitle(
    translate("updateCollectionToMinecraftVersion", "Update {collection} to a Minecraft version")
      .replace("{collection}", resolveTabCollectionLabel(tab))
  );
  const subtitle = createModalSubtitle(translate("bulkUpdateBody", "PackTracker will try to find the newest compatible tracked version for every visible item. If it cannot find one, it will tell you why."));

  const versionGroup = createTextField(translate("targetMinecraftVersion", "Target Minecraft version"));
  ...
  note.textContent = tab === "mods"
    ? translate("modsMatchedAgainstBody", "Mods will be matched against {loader} and the target Minecraft version.")
      .replace("{loader}", capitalize(profile.loader))
    : translate("packsMatchedAgainstBody", "Resource packs and shaders will be matched against the chosen Minecraft version when possible.");

  const preferenceNote = document.createElement("div");
  preferenceNote.className = "modal-subtitle";
  preferenceNote.textContent = `${translate("providerPreference", "Provider preference")}: ${resolveUpdatePreferenceLabel(updatePreference)}.`;
  ...
  const actions = createActionRow();
  const cancelButton = createButton(translate("cancel", "Cancel"));
  const startButton = createButton(translate("startUpdate", "Start update"), "btn-primary");
  cancelButton.addEventListener("click", closeOverlays);
...
async function resolveBulkUpdateProjectForSource(item, projectType, profile, source) {
  ...
  const loader = (projectType === "mod" || projectType === "modpack") && profile.loader !== "vanilla" ? profile.loader : "";
...
function renderBulkUpdateProgressModal() {
  ...
  closeButton.setAttribute("aria-label", translate("closeUpdateProgress", "Close update progress"));
  ...
  doneButton.textContent = translate("done", "Done");
...
async function downloadCategoryAsZip(items, categoryFolderName, profileName, preparedDownload = null) {
  ...
  const [withUrl, withoutUrl] = partition(normalizedItems, (item) => Boolean(resolveStoredDownloadUrl(item)));
...
async function resolveDownloadTarget(item) {
  const source = resolveTrackedItemSource(item);

  if (source === "modrinth" && item.versionId) {
  ...
  const directUrl = resolveStoredDownloadUrl(item);
  if (directUrl) {
    return {
      url: directUrl,
...
function renderDownloadProgressModal() {
  ...
  closeButton.className = "icon-btn";
  closeButton.type = "button";
  closeButton.setAttribute("aria-label", translate("closeDownloads", "Close downloads"));
  closeButton.textContent = "X";
...
function resolveDownloadProjectId(item) {
  if (item.projectId || item.project_id) {
    return String(item.projectId || item.project_id);
  }
  if (item.modrinthId || item.modrinth_id) {
    return String(item.modrinthId || item.modrinth_id);
  }
...
async function resolveZipDownloadTarget(item) {
  ...
  if (versionId) {
    const version = await getVersionForSource(source, versionId);
    if (!version?.error && Array.isArray(version?.files) && version.files.length > 0) {
      const primaryFile = version.files.find((file) => file.primary) || version.files[0];
      return {
...
/**
 * Resolves the most useful known download URL from an item record.
 *
 * @param {object} item - Stored item.
 * @returns {string} Best available direct download URL.
 */
function resolveStoredDownloadUrl(item) {
  const candidates = [
    item?.fileUrl,
    item?.downloadUrl,
    item?.url,
...
function buildProjectUrl(source, projectType, slug, projectId) {
  if (source === "curseforge") {
    const segment = projectType === "modpack"
      ? "modpacks"
...
async function showMissingDependenciesModal(mod, profile, missingDependencies) {
  ...
  modal.classList.add("modal-wide");
  const title = createModalTitle(
    translate("missingDependenciesFor", "Missing dependencies for {name}")
  ...
  unresolvedDependencies.forEach((dependencyId) => {
    ...
    meta.textContent = translate("fetchingModrinthDetails", "Fetching Modrinth details...");

    text.append(name, meta);

    const addButton = createButton(translate("add", "Add"), "btn-accent btn-small");
    addButton.addEventListener("click", async () => {
      addButton.disabled = true;
      addButton.textContent = translate("adding", "Adding...");
      const outcome = await addDependencyToProfile(dependencyId, profile);
      addButton.textContent = outcome
        ? translate("added", "Added")
    ...
    if (cachedProject) {
      ...
      getProject(dependencyId).then((project) => {
        if (!project.error) {
          ...
          meta.textContent = project.message || translate("failedToFetchProjectDetails", "Failed to fetch project details.");
        }
      });
    }
  });
  ...
  const actions = createActionRow();
  const closeButton = createButton(translate("close", "Close"));
...
function showItemMenu(item, profileId, type, x, y) {
  ...
  if (type === "mod") {
    menu.appendChild(
      createContextItem(translate("editNotes", "Edit notes"), () => {
  ...
  const downloadItem = createContextItem("Download file", async () => {
    ...
    downloadItem.textContent = translate("preparing", "Preparing...");
    ...
    try {
      ...
      if (started) {
        ...
        downloadItem.textContent = translate("downloadFile", "Download file");
      }
    } catch (error) {
      downloadItem.disabled = false;
      downloadItem.textContent = translate("downloadFile", "Download file");
      if (typeof namespace.showToast === "function") {
        namespace.showToast(error instanceof Error ? error.message : translate("downloadFailed", "Download failed"), "danger");
      }
    }
  });
  ...
  if (alternateSource) {
    ...
    const switchItem = createContextItem(createExperimentalContextLabel(switchLabel), async () => {
      ...
      switchItem.textContent = translate("switching", "Switching...");
      try {
        ...
        switchItem.replaceChildren(createExperimentalContextLabel(switchLabel));
        if (typeof namespace.showToast === "function") {
          namespace.showToast(error instanceof Error ? error.message : translate("providerSwitchFailed", "Provider switch failed"), "danger");
        }
      }
    });
...
/**
 * Removes one tracked item from a profile based on its stored type.
 *
 * @param {string} profileId - Owning profile identifier.
 * @param {string} itemId - Stored item identifier.
 * @param {"mod"|"resourcepack"|"shader"} type - Stored item type.
 */
function removeTrackedItem(profileId, itemId, type) {
  if (AppState.settings?.confirmItemRemoval) {
    showRemoveTrackedItemModal(profileId, itemId, type);
    return;
  }

  removeTrackedItemNow(profileId, itemId, type);
}

/**
 * Removes one tracked item immediately from a profile based on its stored type.
 *
 * @param {string} profileId - Owning profile identifier.
 * @param {string} itemId - Stored item identifier.
 * @param {"mod"|"resourcepack"|"shader"} type - Stored item type.
 */
function removeTrackedItemNow(profileId, itemId, type) {
  if (type === "mod") {
...
/**
 * Shows a confirmation modal before one tracked item is removed.
 *
 * @param {string} profileId - Owning profile identifier.
 * @param {string} itemId - Stored item identifier.
 * @param {"mod"|"resourcepack"|"shader"} type - Stored item type.
 */
function showRemoveTrackedItemModal(profileId, itemId, type) {
  const profile = AppState.data?.profiles.find((entry) => entry.id === profileId);
  const collection = type === "mod"
    ? profile?.mods
    : type === "resourcepack"
  ...
  const item = Array.isArray(collection)
    ...
    : null;
  const itemName = item?.name || translate("thisItem", "this item");
  const overlay = createModalOverlay();
  const modal = createModalCard();
  const title = createModalTitle(translate("deleteThing", "Delete {name}?").replace("{name}", itemName));
  const subtitle = createModalSubtitle(
    translate("confirmRemoveTrackedItemBody", "This removes the item from the current list. This cannot be undone.")
  );
  const actions = createActionRow();
  const cancelButton = createButton(translate("cancel", "Cancel"));
  const deleteButton = createButton(translate("delete", "Delete"), "btn-danger");
  cancelButton.addEventListener("click", closeOverlays);
  deleteButton.addEventListener("click", () => {
    removeTrackedItemNow(profileId, itemId, type);
    closeOverlays();
...
function showDeleteProfileModal(profileId, name) {
  const overlay = createModalOverlay();
  const modal = createModalCard();
  const title = createModalTitle(translate("deleteThing", "Delete {name}?").replace("{name}", name));
  const subtitle = createModalSubtitle(translate("deleteProfileItemsBody", "This removes the profile and every tracked item from PackTracker."));
  const actions = createActionRow();
  const cancelButton = createButton(translate("cancel", "Cancel"));
  const deleteButton = createButton(translate("delete", "Delete"), "btn-danger");
  cancelButton.addEventListener("click", closeOverlays);
...
/**
 * Shows a confirmation modal for clearing every item in the active tab.
 *
 * @param {string} profileId - Profile identifier.
 * @param {"mods"|"resourcepacks"|"shaders"} tab - Active tab key.
 */
function showClearItemsModal(profileId, tab) {
  const overlay = createModalOverlay();
  const modal = createModalCard();
  const title = createModalTitle(translate("clearItemsTitle", "Clear items?"));
  const subtitle = createModalSubtitle(
    translate("clearItemsBody", "This removes every item from the current list. This cannot be undone.")
  );
  const actions = createActionRow();
  const cancelButton = createButton(translate("cancel", "Cancel"));
  const clearButton = createButton(translate("clearItems", "Clear items"), "btn-danger");
  cancelButton.addEventListener("click", closeOverlays);
  clearButton.addEventListener("click", () => {
    const collectionKey = tab === "mods"
      ? "mods"
      : tab === "resourcepacks"
        ? "resourcePacks"
        : "shaders";
    if (typeof clearProfileCollection === "function") {
      clearProfileCollection(profileId, collectionKey);
    }
    closeOverlays();
  });
...
function mapProjectVersionToMod(project, version, dependencyIds, dependencyProjects = []) {
  ...
  return {
    id: projectId,
    projectId,
    projectType: project?.project_type === "modpack" ? "modpack" : "mod",
    slug,
    name: project.title || project.name || "Unknown mod",
    description: project.description || "",
    author: project.author || project.team || translate("unknownAuthor", "Unknown author"),
    ...
    modrinthUrl: buildProjectUrl(source, project?.project_type === "modpack" ? "modpack" : "mod", slug, projectId),
    ...
    dependencyProjects: Array.isArray(dependencyProjects)
      ...
      : [],
    includedMods: Array.isArray(project?.includedMods) ? project.includedMods : [],
    addedAt: Date.now(),
  };
}

async function buildTrackedModEntryFromProjectVersion(project, version, options = {}) {
  const projectType = options.projectType || project?.project_type || "mod";
  const source = options.source || project?.source || "modrinth";
  if (projectType !== "modpack") {
    return mapProjectVersionToMod(project, version, [], []);
  }

  const includedMods = await resolveModpackIncludedMods(
    {
      source,
      projectType,
      name: project?.title || project?.name || "Unknown modpack",
    },
    version
  );

  return {
    ...mapProjectVersionToMod(
      {
        ...project,
        source,
...
async function resolveModpackIncludedMods(item, version) {
  const source = item?.source || "modrinth";
  const primaryFile = Array.isArray(version?.files)
    ? version.files.find((file) => file.primary) || version.files[0]
    : null;
  const downloadUrl = primaryFile?.url || item?.fileUrl || item?.downloadUrl || "";
  if (!downloadUrl || typeof window.JSZip !== "function") {
    return Array.isArray(item?.includedMods) ? item.includedMods : [];
  }

  try {
    if (source === "curseforge") {
      return await extractCurseForgeModpackMods(downloadUrl);
    }
    return await extractModrinthModpackMods(downloadUrl);
  } catch (error) {
    console.warn("PackTracker: failed to parse modpack contents", error);
    return Array.isArray(item?.includedMods) ? item.includedMods : [];
  }
}

async function extractModrinthModpackMods(downloadUrl) {
  const archive = await fetchZipArchive(downloadUrl);
  const indexFile = archive.file("modrinth.index.json");
  if (!indexFile) {
    return [];
  }
  ...
  return files
    .filter((entry) => String(entry?.path || "").toLowerCase().startsWith("mods/"))
    .map((entry, index) => {
      const fileName = String(entry?.path || "").split("/").pop() || `mod-${index + 1}.jar`;
      const downloadSource = Array.isArray(entry?.downloads) ? entry.downloads[0] : "";
      return {
        id: String(entry?.hashes?.sha1 || entry?.hashes?.sha512 || fileName),
        projectId: "",
        slug: "",
        name: humanizeModFileName(fileName),
        versionId: "",
        versionNumber: "",
        fileName,
        source: downloadSource && downloadSource.includes("curseforge.com") ? "curseforge" : "modrinth",
      };
    });
}

async function extractCurseForgeModpackMods(downloadUrl) {
  const archive = await fetchZipArchive(downloadUrl);
  ...
  const entries = await Promise.all(
    files.map(async (entry, index) => {
      ...
      const versionId = String(entry?.fileID || "");
      let name = `Project ${projectId || index + 1}`;
      try {
        const project = await cfGetProject(projectId);
        if (!project?.error) {
          name = project.title || project.name || name;
        }
      } catch (error) {
        // Ignore individual project lookup failures.
      }
      return {
        id: `${projectId}:${versionId}`.replace(/^:/, versionId || `cf-${index + 1}`),
        projectId,
        slug: "",
        name,
        versionId,
...
async function fetchZipArchive(downloadUrl) {
  const response = await fetch(downloadUrl);
  if (!response.ok) {
    throw new Error(`Could not fetch modpack archive (${response.status}).`);
  }
  const buffer = await response.arrayBuffer();
  return window.JSZip.loadAsync(buffer);
}

function humanizeModFileName(fileName) {
  return String(fileName || "Unknown mod")
    .replace(/\.(jar|zip)$/i, "")
    .replace(/[-_]+/g, " ")
    .replace(/\s+/g, " ")
    .trim() || "Unknown mod";
...
function createIncludedModsSection(includedMods) {
  const details = document.createElement("details");
  details.className = "included-mods";

  const summary = document.createElement("summary");
  summary.className = "included-mods-summary";
  summary.textContent = translate("containsMods", "Contains {count} mods").replace("{count}", String(includedMods.length));
  details.appendChild(summary);

  const list = document.createElement("div");
  list.className = "included-mods-list";
  includedMods.forEach((entry) => {
    const row = document.createElement("div");
    row.className = "included-mod-row";
    row.textContent = entry.versionNumber
...
function findRecommendedVersion(versions, profile, projectType) {
  ...
  const preferred = safeVersions.find((version) => {
    ...
    const matchesLoader = (projectType !== "mod" && projectType !== "modpack")
      || profile.loader === "vanilla"
...
function inferProjectTypeFromItem(item) {
  if (item?.projectType === "resourcepack" || item?.projectType === "shader" || item?.projectType === "mod" || item?.projectType === "modpack") {
...
function showUpdateProviderPreferenceModal(options = {}) {
  const overlay = createModalOverlay();
  const modal = createModalCard();
  const title = createModalTitle(translate("updateProviderPreference", "Update provider preference"));
  const subtitle = createModalSubtitle(translate("updateProviderPreferenceBody", "Choose which source PackTracker should prefer first when checking for newer compatible versions."));
  ...
  UPDATE_PROVIDER_PREFERENCES.forEach((optionData) => {
    ...
    meta.textContent = optionData.value === "auto"
      ? translate("updateProviderAutoBody", "Use the current item source first, then try the other provider if needed.")
      : translate("updateProviderSpecificBody", "Try {source} first, then fall back if no compatible match exists.")
        .replace("{source}", resolveSourceLabel(optionData.value));
  ...
  const actions = createActionRow();
  const cancelButton = createButton(translate("cancel", "Cancel"));
  const saveButton = createButton(translate("savePreference", "Save preference"), "btn-primary");
  ...
  saveButton.addEventListener("click", () => {
    ...
    if (typeof namespace.showToast === "function") {
      namespace.showToast(`${translate("updatePreferenceSaved", "Update preference saved")}: ${resolveUpdatePreferenceLabel(selectedValue)}.`, "success");
    }
    closeOverlays();
    if (onClose) {
      onClose();
...
function mountModal(overlay) {
  ...
  overlay.addEventListener("click", (event) => {
    if (overlay.dataset.closeOnAnyClick === "true" || event.target === overlay) {
      closeOverlays();
    }
  });
...
function createExperimentalContextLabel(text) {
  ...
  tag.className = "experimental-tag";
      tag.textContent = translate("experimental", "Experimental");
...
/**
 * Toggles the current tab's drag-order edit mode.
 *
 * @param {"mods"|"resourcepacks"|"shaders"} [tab] - Active tab id.
 * @returns {boolean} True when the mode changed.
 */
function toggleLayoutEditMode(tab = AppState.activeTab) {
  if (!Object.prototype.hasOwnProperty.call(layoutEditState, tab)) {
    return false;
  }

  layoutEditState[tab] = !layoutEditState[tab];
  if (typeof notifyStateChanged === "function") {
    notifyStateChanged("layout-edit");
  }
  return true;
}

/**
 * Clears every item from the current tab on the active profile.
 *
 * @param {"mods"|"resourcepacks"|"shaders"} [tab] - Active tab id.
 * @returns {boolean} True when the collection was cleared.
 */
function clearActiveTabItems(tab = AppState.activeTab) {
  const profile = getActiveProfile();
  if (!profile || isFavoritesProfileId(AppState.activeProfileId)) {
    return false;
  }
  ...
  const collectionKey = tab === "mods"
    ...
    : tab === "resourcepacks"
      ? "resourcePacks"
      : "shaders";
  const cleared = typeof clearProfileCollection === "function"
    ? clearProfileCollection(profile.id, collectionKey)
    : false;
  if (cleared && typeof notifyStateChanged === "function") {
    notifyStateChanged("clear-items");
  }
  return cleared;
}

/**
 * Opens the normal clear-items confirmation modal for the active profile/tab.
 *
 * @param {"mods"|"resourcepacks"|"shaders"} [tab] - Active tab id.
 * @returns {boolean} True when a confirmation modal was opened.
 */
function showClearActiveTabItemsModal(tab = AppState.activeTab) {
  const profile = getActiveProfile();
  if (!profile || isFavoritesProfileId(AppState.activeProfileId)) {
    return false;
  }
...
Object.assign(namespace, {
  ...
  buildTrackedModEntryFromProjectVersion,

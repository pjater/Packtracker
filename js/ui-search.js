...
const MODS_BROWSE_TABS = [
  { key: "mods", label: "Mods", projectType: "mod" },
  { key: "modpacks", label: "Modpacks", projectType: "modpack" },
];
...
function renderSearchPage() {
  ...
  const isModsBrowsePage = AppState.browseContext?.defaultTab === "mods";
  const isValidModsBrowseType = AppState.search.projectType === "mod" || AppState.search.projectType === "modpack";
  if (desiredProjectType && ((!isModsBrowsePage && AppState.search.projectType !== desiredProjectType) || (isModsBrowsePage && !isValidModsBrowseType))) {
  ...
  header.append(titleRow, renderSourceToggle());
  if (browseTabs) {
    header.appendChild(browseTabs);
  }
  ...
  const activeLoader = isLoaderAwareProjectType(AppState.search.projectType) ? AppState.search.loader : "";
...
function renderSearchCard(project) {
  ...
  const addButton = createButton(resolveAddButtonLabel(project), "btn-accent");
  ...
  const sourcePresentation = resolveSourcePresentation(project.source, { includeArrow: true });
  sourceLinkButton.textContent = sourcePresentation.label;
  sourceLinkButton.setAttribute("aria-label", sourcePresentation.title);
  sourceLinkButton.title = translate("openProjectPage", "Open project page");
...
async function resolveAndAddMod(project, version, profileId, options = {}) {
  ...
  if (projectType !== "mod") {
    if (projectType === "modpack") {
      const trackedEntry = typeof namespace.buildTrackedModEntryFromProjectVersion === "function"
        ? await namespace.buildTrackedModEntryFromProjectVersion(project, version, {
            projectType,
            source,
          })
        : mapProjectVersionToMod(project, version, [], source);
      const savedItem = addMod(profileId, trackedEntry);
...
function findDuplicateTrackedItem(profile, project, projectType) {
  ...
  return findTrackedProjectMatch(project, Array.isArray(collection) ? collection : [], projectType);
}
...
function renderFilterRow() {
  ...
  if (isLoaderAwareProjectType(AppState.search.projectType)) {
    row.append(
...
function renderSourceToggle() {
  ...
  [
    ...
    button.type = "button";
    const sourcePresentation = resolveSourcePresentation(source.value);
    button.textContent = sourcePresentation.label;
    button.title = sourcePresentation.title;
...
function renderBrowseTabs() {
  if (AppState.browseContext?.defaultTab !== "mods") {
    return null;
  }
  ...
  MODS_BROWSE_TABS.forEach((tab) => {
    const button = document.createElement("button");
    button.className = AppState.search.projectType === tab.projectType ? "tab active" : "tab";
    button.type = "button";
      ...
      setSearchState(
        {
          ...
          loader: isLoaderAwareProjectType(tab.projectType) ? AppState.search.loader : "",
          results: [],
          offset: 0,
          loading: true,
        },
...
function formatLoaderSummary(project) {
  if ((project.project_type || AppState.search.projectType) === "modpack") {
    return "Modpack";
  }
 ...
 */
function resolveAddButtonLabel(project) {
  const projectType = project?.project_type || AppState.search.projectType || "mod";
  return projectType === "modpack" ? "+ Add modpack to profile" : "+ Add to profile";
}
...
async function handleBrowseMultiSelectAdd(project) {
  ...
  if (trackedItem) {
    if (trackedItem.matchKind === "included-modpack") {
      if (typeof namespace.showToast === "function") {
        namespace.showToast(`Already included via modpack: ${trackedItem.parentName}.`, "warning");
      }
      return;
    }
...
function findTrackedProjectMatch(project, collection, projectType) {
  ...
  for (const entry of collection) {
    if (matchesTrackedProjectEntry(entry, matcher)) {
      return entry;
    }

    if (projectType === "mod") {
      const includedMods = Array.isArray(entry?.includedMods) ? entry.includedMods : [];
      const includedMatch = includedMods.find((includedEntry) => matchesTrackedProjectEntry(includedEntry, matcher));
      if (includedMatch) {
        return {
          ...entry,
          matchKind: "included-modpack",
          matchedIncludedMod: includedMatch,
          parentId: entry.id,
...
function createProjectMatcher(project) {
...
function matchesTrackedProjectEntry(entry, matcher) {
  ...
  return (matcher.projectId && entryId === matcher.projectId)
    || (matcher.slug && entrySlug === matcher.slug)
    || (matcher.name && entryName === matcher.name);
}
...
function mapProjectVersionToMod(project, version, dependencyIds, source = AppState.searchSource || "modrinth", dependencyProjects = []) {
  ...
  return {
    ...
    downloadUrl: primaryFile?.url || "",
    modrinthId: projectId,
    modrinthUrl: buildProjectUrl(source, project.project_type === "modpack" ? "modpack" : "mod", project.slug || projectId, projectId),
    iconUrl: project.icon_url || "",
    source,
    starred: false,
    notes: "",
...
function isLoaderAwareProjectType(projectType) {
...
function resolveActiveSourceLabel() {
  return resolveSourcePresentation(AppState.searchSource).title;
}

/**
 * Resolves one addon-aware source label for browse controls.
 *
 * @param {string} source - Source identifier.
 * @param {{includeArrow?: boolean}} [options] - Optional label options.
 * @returns {{label:string,title:string}} Label payload.
 */
function resolveSourcePresentation(source, options = {}) {
  if (typeof namespace.formatAddonSourceLabel === "function") {
    return namespace.formatAddonSourceLabel(source, options);
  }
  const safeSource = String(source || "modrinth").trim().toLowerCase() === "curseforge" ? "curseforge" : "modrinth";
  const title = safeSource === "curseforge" ? "CurseForge" : "Modrinth";
  return {
    label: options.includeArrow ? `${title} ↗` : title,
    title,
  };
}

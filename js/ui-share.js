(function attachShareModule() {
  ...
  function renderShareView() {
    ...
    meta.append(
      createMetaBadge("badge-source", translate("sharedProfile", "Shared profile")),
      createMetaBadge("badge-version", profile.mcVersion || translate("unknown", "Unknown")),
      createLoaderBadge(profile.loader || "vanilla")
    );

    const subtitle = document.createElement("div");
    subtitle.className = "share-subtitle";
    subtitle.textContent = `${profile.mods.length} ${translate("modsLower", "mods")} • ${profile.resourcePacks.length} ${translate("resourcePacksLower", "resource packs")} • ${profile.shaders.length} ${translate("shadersLower", "shaders")}`;
    ...
    const backButton = createButton(translate("backToApp", "Back to app"));
    backButton.addEventListener("click", () => {
      closeShareDownloadView();
      setActiveView("home");
    });

    const zipButton = createButton(translate("downloadAsZip", "⬇ Download as ZIP"), "btn-primary");
    ...
    SHARE_TAB_DEFINITIONS.forEach((definition) => {
      ...
      tabButton.textContent = definition.key === "resourcePacks"
        ? translate("resourcePacks", "Resource Packs")
        : translate(definition.key, definition.label);
    ...
    if (items.length === 0) {
      ...
      empty.textContent = translate("sharedProfileEmptyTab", "This shared profile has no items in the current tab.");
    ...
    const importButton = createButton(translate("importToMyProfiles", "Import to my profiles"), "btn-primary");
  ...
  function showShareImportModal(sharedProfile) {
    ...
    title.textContent = translate("importSharedProfile", "Import shared profile");

    const subtitle = document.createElement("div");
    subtitle.className = "modal-subtitle";
    subtitle.textContent = translate("importSharedProfileBody", "You can import this pack into your own library or open a read-only download page first.");

    const summary = document.createElement("div");
    summary.className = "share-summary";
    summary.append(
      createSummaryRow(translate("profile", "Profile"), profile.name),
      createSummaryRow(translate("minecraft", "Minecraft"), profile.mcVersion || translate("unknown", "Unknown")),
      createSummaryRow(translate("loader", "Loader"), capitalize(profile.loader || "vanilla")),
      createSummaryRow(translate("contents", "Contents"), `${profile.mods.length} ${translate("modsLower", "mods")}, ${profile.resourcePacks.length} ${translate("resourcePacksLower", "resource packs")}, ${profile.shaders.length} ${translate("shadersLower", "shaders")}`)
    );
    ...
    const downloadOnlyButton = createButton(translate("downloadOnly", "Download only"));
    downloadOnlyButton.addEventListener("click", () => {
      closeModal();
      openShareDownloadView(profile);
    });

    const importButton = createButton(translate("importProfile", "Import profile"), "btn-primary");
    importButton.addEventListener("click", () => {
      const importedProfile = importSharedProfile(profile);
      closeModal();
      setActiveProfile(importedProfile.id);
  ...
  function renderShareItem(item) {
    ...
    name.textContent = item.name || item.slug || translate("unknown", "Unknown");

    const meta = document.createElement("div");
    meta.className = "share-item-meta";
    meta.textContent = [
      item.versionNumber || item.versionId || translate("unknown", "Unknown"),
    ...
    const downloadButton = createButton(translate("download", "Download"));
    const fileUrl = String(item.fileUrl || "");
    if (!fileUrl) {
      downloadButton.disabled = true;
      downloadButton.textContent = translate("noLink", "No link");
    } else {
